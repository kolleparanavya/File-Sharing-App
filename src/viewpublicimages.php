...
<body>
  ...
  <div class="dropdown">
    <select name="navigate" onchange="location = this.value;">
      ...
      <option value="viewimages.php">Move to Private Interface</option>
...
<table border="2px">
  ...
  $sql = "SELECT image FROM publicimage";
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        foreach ($files as $file) {
            ...
            if ($file_name == $name) {
                ...
                echo '<td><center> <a href="'.$file.'" target="_blank"> <img src="image.jpg" alt="PPT icon" style="width:90px;height:90px;"> <br> <font color="black">'.str_ireplace($search, "<mark>$search</mark>", $file_name).'</font></a><center></td>';
                echo '<td><center><a href="'.$file.'" download>Download</center></a></td>';
                echo '<td><center>';
                echo '<a href="deletepublicimage.php?file_name='.$file_name.'"><font color = "red">Delete</font></a>';
                echo '</center></td>';
                echo '</tr>';
                $i++;
            }
