...
<table border="2px">
  ...
  $sql = "SELECT music FROM publicmusic";
  ...
  if (mysqli_num_rows($result) > 0) {
    ...
    while($row = mysqli_fetch_assoc($result)) {
        ...
        foreach ($files as $file) {
            ...
            if ($file_name == $name) {
                ...
                echo '<td><center> <a href="'.$file.'" target="_blank"> <img src="music.png" alt="PPT icon" style="width:90px;height:90px;"> <br> <font color="black">'.str_ireplace($search, "<mark>$search</mark>", $file_name).'</font></a><center></td>';
                echo '<td><center><a href="'.$file.'" download>Download</center></a></td>';
                echo '<td><center>';
                echo '<a href="deletepublicmusics.php?file_name='.$file_name.'"><font color = "red">Delete</font></a>';
                echo '</center></td>';
                echo '</tr>';
                $i++;
            }
    ...
    echo "No Music found for the current user";
}
mysqli_close($conn);
?>
</table>
