...
<body>
    <style>
        ...
        <div class="otherfiles">
            ...
            <center>
            <a href="http://localhost:3000/src/viewpublicpdfs.php"><button class="publicbutton">View Public Interface</button></a>
...
<div class="search-form">
        <form action="" method="get">
            <input type="text" name="search" placeholder="Search Your PDF file" value="<?php if(isset($_GET['search'])) echo $_GET['search']; ?>">
...
<table border="2px">
  ...
  $sql = "SELECT pdf FROM pdfs WHERE userid = '$user'";
  ...
  if (mysqli_num_rows($result) > 0) {
    while($row = mysqli_fetch_assoc($result)) {
        ...
        foreach ($files as $file) {
            ...
            if ($file_name == $name) {
                ...
                echo '<td><center> <a href="'.$file.'" target="_blank"> <img src="pdficon.png" alt="PDF icon" style="width:90px;height:90px;"> <br> <font color="black">'.str_ireplace($search, "<mark>$search</mark>", $file_name).'</font></a><center></td>';
                echo '<td><center><a href="'.$file.'" download>Download</center></a></td>';
                // Generate a public link using the file path or a URL
                echo '<td><center>';
                echo '<a href="deletepdf.php?file_name='.$file_name.'"><font color = "red">Delete</font></a>';
                echo '</center></td>';
                echo '<td> <center><a href="pinsertpdf.php?file_name='.$file_name.'" onclick="return confirm(\'Are you sure you want to move the file to public interface?\');">move the file to public interface</a></center></td>';
                echo '</tr>';
                $i++;
            }
        }
    ...
    echo "No PDFs found for the current user";
}
?>
</table>
</center>
